// a.c
// Dock-like X11 status bar (fixed: pure C, no lambdas)
// - dwm-style tags, hide empty tags
// - inotify for immediate updates
// - clickable tags to switch workspace via command (env XSTATUS_WS_SWITCH_CMD)
// - content-width window centered at top (empty sides = actual desktop visible)
// - uses Xft; default font xterm-12; fallback to monospace-12
//
// build:
// gcc -O2 -Wall -o x11_status_bar a.c -I/usr/include/freetype2 -lX11 -lXft -lfontconfig -lm
...
static Window g_win;
static TagRect *g_tagrects = NULL;
...
static void draw_all(void) {
    ...
    if (g_cmd) {
        ...
        if (f) {
            if (fgets(status_text, sizeof(status_text), f)) {
                size_t L=strlen(status_text); if (L && status_text[L-1]=='\n') status_text[L-1]='\0';
    ...
    /* occupied */
    int *occupied = calloc(g_ws_count+1, sizeof(int));
    if (g_occupied_path[0]) {
        ...
        while (*p) {
            ...
            if (!*p) break;
            int n = atoi(p);
            if (n >= 1 && n <= g_ws_count) occupied[n]=1;
            while (*p && !isspace((unsigned char)*p)) ++p;
        }
    }
    ...
    int x = PADDING;
    free(g_tagrects);
    g_tagrects = malloc(sizeof(TagRect) * g_ws_count);
    g_tagrects_n = 0;
    for (int i=1;i<=g_ws_count;++i) {
        if (!occupied[i]) continue;
        char tb[8]; snprintf(tb, sizeof(tb), "%d", i);
        XGlyphInfo ginfo;
    ...
    /* clear window background with bg_pixel for the content area (leave sides transparent/invisible) */
    GC gc = XCreateGC(g_dpy, g_win, 0, NULL);
    XSetForeground(g_dpy, gc, g_bg_pixel);
    XFillRectangle(g_dpy, g_win, gc, 0, 0, content_w, g_bar_h);
    XFreeGC(g_dpy, gc);
    ...
    for (int i=0;i<g_tagrects_n;++i) {
        ...
        char tb[8]; snprintf(tb,sizeof(tb),"%d",tag);
        int tx = g_tagrects[i].x;
        int w = g_tagrects[i].w;
        if (tag == focused_ws) {
            /* focus bg */
            GC gc2 = XCreateGC(g_dpy, g_win, 0, NULL);
            XSetForeground(g_dpy, gc2, g_xc_focus.pixel);
            int ry = (g_bar_h - (g_font->ascent + g_font->descent))/2 - 2;
            if (ry < 0) ry = 0;
            XFillRectangle(g_dpy, g_win, gc2, tx-2, ry, w+4, g_font->ascent + g_font->descent + 4);
            XFreeGC(g_dpy, gc2);
            XftDrawStringUtf8(g_draw, &g_xft_focus_text, g_font, tx + TAG_PADDING/2, text_y, (FcChar8*)tb, strlen(tb));
        } else {
            XftDrawStringUtf8(g_draw, &g_xft_fg, g_font, tx + TAG_PADDING/2, text_y, (FcChar8*)tb, strlen(tb));
        }
    }
    ...
    XftDrawStringUtf8(g_draw, &g_xft_shadow, g_font, status_x + status_off + 1,
                      text_y + 1, (FcChar8*)status_text, strlen(status_text));
    XftDrawStringUtf8(g_draw, &g_xft_fg, g_font, status_x + status_off,
                      text_y, (FcChar8*)status_text, strlen(status_text));
    XFlush(g_dpy);
...
int main(void) {
    ...
    } else { g_focused_path[0]=g_occupied_path[0]=0; }

    /* inotify */
    int inofd = inotify_init1(IN_NONBLOCK | IN_CLOEXEC);
    int watch_focused = -1, watch_occupied = -1;
    if (inofd >= 0) {
        if (g_focused_path[0]) watch_focused = inotify_add_watch(inofd, g_focused_path, IN_MODIFY|IN_CREATE|IN_DELETE|IN_MOVED_TO|IN_MOVED_FROM);
        if (g_occupied_path[0]) watch_occupied = inotify_add_watch(inofd, g_occupied_path, IN_MODIFY|IN_CREATE|IN_DELETE|IN_MOVED_TO|IN_MOVED_FROM);
    }
    ...
    g_screen_w = DisplayWidth(g_dpy, g_scr);
    g_bar_h = atoi(env_or("XSTATUS_HEIGHT","28")); if (g_bar_h <= 0) g_bar_h = 28;

    /* colors */
    if (!parse_color(g_dpy, g_cmap, bg_spec, &g_xc_bg)) { g_xc_bg.red = g_xc_bg.green = g_xc_bg.blue = 0xffff; g_bg_pixel = WhitePixel(g_dpy, g_scr); }
    else g_bg_pixel = g_xc_bg.pixel;
    if (!parse_color(g_dpy, g_cmap, fg_spec, &g_xc_fg)) g_xc_fg.red = g_xc_fg.green = g_xc_fg.blue = 0x0000;
    if (!parse_color(g_dpy, g_cmap, focus_spec, &g_xc_focus)) { g_xc_focus.red=0x1e00; g_xc_focus.green=0x9000; g_xc_focus.blue=0xff00; XAllocColor(g_dpy,g_cmap,&g_xc_focus); }
    ...
    if (!g_font) { fprintf(stderr,"failed to open Xft font; try installing fonts or set XSTATUS_FONT to a valid Fc name\n"); XCloseDisplay(g_dpy); return 1; }

    XRenderColor rc_fg = { (unsigned short)g_xc_fg.red, (unsigned short)g_xc_fg.green, (unsigned short)g_xc_fg.blue, 0xffff };
    if (!XftColorAllocValue(g_dpy, vis, g_cmap, &rc_fg, &g_xft_fg)) { XRenderColor fb = {0,0,0,0xffff}; XftColorAllocValue(g_dpy,vis,g_cmap,&fb,&g_xft_fg); }

    XRenderColor rc_sh;
    if ((rc_fg.red>0x7fff) && (rc_fg.green>0x7fff) && (rc_fg.blue>0x7fff)) rc_sh.red=rc_sh.green=rc_sh.blue=0x0000;
    else rc_sh.red=rc_sh.green=rc_sh.blue=0xffff;
    rc_sh.alpha = 0x8000;
    ...
    if (lum_focus > 0.5) rc_focus_text.red = rc_focus_text.green = rc_focus_text.blue = 0x0000;
    else rc_focus_text.red = rc_focus_text.green = rc_focus_text.blue = 0xffff;
    ...
    if (a_type && a_type_dock) XChangeProperty(g_dpy, g_win, a_type, XA_ATOM, 32, PropModeReplace, (unsigned char *)&a_type_dock, 1);
    ...
    pfds[0].fd = ConnectionNumber(g_dpy); pfds[0].events = POLLIN;
    int nfds = 1;
    if (inofd >= 0) { pfds[1].fd = inofd; pfds[1].events = POLLIN; nfds = 2; }
